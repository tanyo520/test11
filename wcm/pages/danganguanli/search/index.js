// var hvObj = inbiz("UnifiedSearch6787897f88a14741bbdb0ba3b0c07481"); 
...
require(["jquery", "inbizsdk","pluginsPath/UnifiedSearch/Scripts/insight-search/utils/index"], function ($, inbizsdk,utils) { 
	...
	hvObj.afterEvent = function () { 
		...
		hvObj.$model.vf.addContentRender(function(element,data){				
			...
			if(typeof(filenames) != "undefined"){
				...
				for(var i=0;i<length;i++){	
					Url+="<a target=view_window href='http://"+window.location.host+"/preview.html?fileid="+data.RawData._source.files[i].fileid+"'>"+data.RawData._source.filenames[i].filename+"</a>&nbsp;&nbsp;"
				}
			}									
		...
		setTimeout(function(){
			var myHeight = getClientHeight() - 222;	
			hvObj.$model.vf.setListHeight(myHeight)
		},0)
