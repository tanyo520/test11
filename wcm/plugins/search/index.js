define(["knockout", "jquery", "inbizsdk"], function (ko, $, inbizsdk) {
    ...
    function addcollection() {
        ...
        for (var i = 0; i < selectedRows.length; i++) {
            ...
            inbizsdk.$dataset("CountMyfavoritesByCreateIdAndArchId", param, function (result) {
                if ((result.Data[0][0].count - 0) > 0) {
                    message = message  + name + "已经加入收藏，不能重复加入！<br>";
                }else {

                    message2 +=  ","  + selectedRows[i].RawData._source.name ;
        ...
        if (values) {
            ...
            inbizsdk.$dataset("BatchInsertMyfavorites", batchParam, function (result) {
                ...
                var arr = message2.split(',');


                if (result.EffectOfRow > 0) {
                    for (var i=0;i<arr.length;i++){
                        message3+=arr[i]+ ',收藏成功！<br>';
                    }
                    //message2 += '收藏成功！';
                } else {
                    for (var i=0;i<arr.length;i++){
                        message3+=arr[i]+ ',收藏失败！<br>';
                    }
                    //message2 += '收藏失败！';
                }
            }, false);
        ...
        if(message){
            message = '以下档案:<br>'+message;
        }
        if(message3){
            message3 = '以下档案:<br>'+message3;
        }

        if(count==selectedRows.length){
            inbizsdk.$components.tip.success("收藏成功");
        }else if(count==0){
            inbizsdk.$components.tip.error("选中的档案已经加入收藏,不可重复加入！");
        }else {
             AllMessage = message + message3;
            inbizsdk.$components.tip.error(AllMessage,true);
        }
    }
    ...
    function useBorrow() {
       ...
       var paramStr= JSON.stringify(list);*/
        ...
        var newFormUrl = "/eform/Default/default?formId=" + formId + "&skin=techblue&processId=" + processId + "&taskType=begintask&formver=0&borrowType=first&archInfoId="+archId+"&archTypeId="+archTypeId+"&formver=0";   //ui页面的存放地址,add-file-copy为重构后的页面
    ...
    function adduse() {
        ...
        for (var i = 0; i < selectedRows.length; i++) {
            ...
            inbizsdk.$dataset("CountMyfavoritesByCreateIdAndArchiveId", param, function (result) {
                if (result.Data[0][0].count - 0 > 0) {
                    ...
                    msg2 += "," + selectedRows[i].RawData._source.name;
        ...
        if (values){
            ...
            inbizsdk.$dataset("BatchInsertBorrowCar", batchParam, function (result) {
                ...
                if(msg2){
                    ...
                    if (result.EffectOfRow > 0) {
                        for (var i=0;i<arr.length;i++){
                            msg3+=arr[i]+ ',加入借阅车成功！<br>';
                        }
                        //message2 += '收藏成功！';
                    } else {
                        for (var i=0;i<arr.length;i++){
                            msg3+=arr[i]+ ',加入借阅车失败！<br>';
                        }
                        //message2 += '收藏失败！';
                    }
                }
        ...
        if(count==selectedRows.length){
            inbizsdk.$components.tip.success("加入借阅车成功");
        }else if(count==0){
            inbizsdk.$components.tip.error("选中的档案已经加入借阅车,不可重复加入！");
        }else {
            AllMsg = msg + msg3;
            inbizsdk.$components.tip.error(AllMsg,true);
        }
