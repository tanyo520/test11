define(["jquery",
    ...
    function ($, p, ko, scrollbar, SearchInput, SearchLabel, inbizsdk) {
            ...
            var vf = {
                ...
                searchChangeLabel: function (data) {
                    ...
                    if (data) {
                        ...
                        if (rowItem.length > 0) {
                            ...
                            var sectid="";
                            var sectName="";
                            var setArchTypeString="";
                            for (var i = 0; i < rowItem.length; i++) {
                                if (rowItem[i]["key"] == "archtypename") {
                                    setArchTypeString=rowItem[i]["key"];
                                    archtypeid = rowItem[i].typeId;
                                    archTypeName = rowItem[i].value;

                                }else if (rowItem[i]["key"] == "sectname"){
                                    setArchTypeString=rowItem[i]["key"];
                                    sectid=rowItem[i].typeId;
                                    sectName=rowItem[i].value;
                                }
                            ...
                            if (archtypeid == "") {
                                $('#formContainer').html('');
                                $("#sectArchTypeTree").combotree({value: sectid});
                                $('.form-search-row').find('.content .treeChooseType').val(setArchTypeString);
                                $('#sectArchTypeTree+.textbox').find('input[type="hidden"]').val(sectid);
                                loadDefatulForm();
                                for (var i = 0; i < rowItem.length; i++) {
                                    if (rowItem[i].type == "time") {
                                        var values = rowItem[i].value.split(',');
                                        var ids = rowItem[i].labelId.split(',');
                                        $('#'+ ids[0]).datebox('setValue', values[0]);
                                        $('#'+ ids[1]).datebox('setValue', values[1]);
                                    }else if(rowItem[i].typeName!=undefined &&  rowItem[i].typeName== "search-type"){
                                        $('#'+rowItem[i].key).combobox('setValue', rowItem[i].value);
                                    }
                                    else {
                                ...
                                $('.form-search-row').find('.content .treeChooseType').val(setArchTypeString);
                                $('#sectArchTypeTree+.textbox').find('input[type="hidden"]').val(archtypeid);
                                //如果有档案类型ID or全宗ID
                                $('#formContainer').html('');
                ...
                saveSearchLabel: function () {
                    ...
                    if (formValue.length) {
                        ...
                        inbizsdk.$components.tip.info("请填写搜索条件");
                        // alert("请填写搜索条件");
                    }
                },
                ...
                search: function () {
                    ...
                    if(labelValue.length==undefined && labelValue==false)
                    {
                        inbizsdk.$components.tip.info("请完善开始日期或结束日期");
                        return;
                    }
                    var searchWhere = "";
                ...
                getFormLabelValue: function () {
                    ...
                    for (var i = 0; i < rowItemContent.length; i++) {
                        ...
                        if (className.indexOf("easyui-combotree")>-1) {
                            ...
                            }else if( (dateArray[0].value != "" && dateArray[1].value == "" )
                            || (dateArray[0].value == "" && dateArray[1].value != "")){
                                // inbizsdk.$components.tip.info("请完善开始日期或结束日期");
                                return false;
                            }
                        }else if(className.indexOf("search-type")>-1){
                            if(inputValue!='')
                            {
                                arryValue.push({
                                    key: elementId,
                ...
                resetValue: function () {
                    ...
                    for (var i = 0; i < contentHtml.length; i++) {
                        if (contentHtml[i].firstElementChild.className.indexOf("easyui-combotree")>-1) {
                            ...
                            $('.form-search-row').find('.content .treeChooseType').val('');
                            $('#sectArchTypeTree+.textbox').find('input[type="hidden"]').val('');
                        ...
                        } else if (contentHtml[i].firstElementChild.className.indexOf("search-type")>-1) {
                            var eleId = contentHtml[i].firstElementChild.id;
                            $('#' +eleId).combobox('setValue', '');
                        } else {
        ...
        function CustomSearch() {
            ...
            /**start penglin 增加授权判断 如果授权是edrms 则显示文档搜索***/
            if(window.globalvalue_ermsVersion!=undefined && window.globalvalue_ermsVersion=="edrms")
            {
                arry.push({key: "文档", value: "文档", isChecked: false, type: "dms"})
            }
            /**end penglin 增加授权判断 ***/
        ...
        pt.setInitialValue = function () {
            ...
            /**start penglin 增加授权判断 如果授权不是 edrms 则隐藏文档搜索 不显示下拉效果***/
            if(window.globalvalue_ermsVersion==undefined || window.globalvalue_ermsVersion!="edrms")
            {
                this.searchInput().vm.showSelect(false);
            }
            /**end penglin 增加授权判断 ***/
        ...
        pt.compositionComplete = function (view) {
            ...
            setTimeout(function () {
                ...
                if(window.globalvalue_ermsVersion==undefined || window.globalvalue_ermsVersion!="edrms")
                {
                    $(".search-custom-view .searchKeywords").addClass("no-Select-searchInput");
                    $(".search-custom-view .hotwords-ul").addClass("no-Select");
                }
                /**end penglin 增加授权判断 ***/
            ...
            $(document).keyup(function(event){
                if(event.keyCode ==13
                    ...
                    if( document.getSelection().focusNode.offsetParent.className.indexOf('orm-search-row')>-1)
                    {
                        current.vf.search();
                    }
