...
window.myExtCode.jumpUrl = function(type, proccessId, taskId, incidentId, businessKey, state) {
	...
	$.ajax({
		type: "get",
		url: "/edoc2Flow-web/edoc2-form/jumpIndex?skin="+skin+"&orgToken=" + $.cookie("token") + "&t=" + new Date().getTime(),
		async: true,
		data: data,
...
window.myExtCode.jump = function(type, proccessId, taskId, incidentId, businessKey, state,formId,formVersion) {
		if(!taskId || taskId == "undefined") {
		taskId = "";
	}
	if(!incidentId || incidentId == "undefined") {
	...
	if(!formVersion || formVersion == "undefined") {
		formVersion = "";
	}

		
		var url = "/eform/Default/default?formId="+formId+"&skin="+skin+"&processId="+proccessId+"&taskType="+type+"&formver="+formVersion+"&taskId="+taskId+"&id="+businessKey+"&incidentId="+incidentId+"&taskState="+state;

		if(eform.pageType == "mobile") {
		
		url = "/eform/mindex?formId="+formId+"&skin="+skin+"&processId="+proccessId+"&taskType="+type+"&formver="+formVersion+"&taskId="+taskId+"&id="+businessKey+"&incidentId="+incidentId+"&taskState="+state;
		}
	...
	efListGrid.method("customColumnsFormatter", function(value, row, index, field, fieldName) {
		if(eform.pageType == "pc") {
			if(field == "Process_Name") {
				...
				if(row.Form_Id){
				 var params =  row.Process_ID + "','" + row.Task_Id + "','" + row.Incident_ID + "','" + row.Process_FormKey + "','" + row.Task_State+"','"+row.Form_Id+"','"+row.Form_Version;
						//我的代办row.Task_Id为何变小写，其他地方都是大写
				return "<a class='blackA' href=\"#\" onclick=\"javascript:window.myExtCode.jump('InboxTask','" + params + "')\">" + value + "</a>";
			...
			if(field == "Process_Name") {
				if(row.Form_Id){
					var params =  row.Process_ID + "','" + row.Task_Id + "','" + row.Incident_ID + "','" + row.Process_FormKey + "','" + row.Task_State+"','"+row.Form_Id+"','"+row.Form_Version;
					return "<a class='blackA' href=\"#\" onclick=\"javascript:window.myExtCode.jump('InboxTask','" + params + "')\" style='color:#009cd0' >" + value + "</a>";
	...
	if(eform.pageType == "mobile") {
		...
		efListGrid.method("getControl").toolbarDrawAfter = function(jqToolbar) {
			try{
				...
				jqToolbar.find(".ui-icon-custom").eq(2).hide();
			}catch(e){}
		};
		...
		efListGrid.method("appendRowsAfter", function(container, rows, allRows) {
			$.each(rows, function(index, row) {
				...
				jqImg.attr("src", $.format("/ImageType/GetUserAvatar?token={0}&userId={1}", $.cookie("token"),rows[index].Task_AssigneeIdentityID)).css({ "border-radius": "6px" });
...
window.myExtCode.parserBefore = function() {
	if(eform.pageType == "pc") {
		...
		eform("edoc2ListGrid_myInbox").method("onBeforeEasyuiControlCreate",function (easyuiControlOptions) {
			var buttons =  easyuiControlOptions.toolbar;
			for(var i=0;i<buttons.length;i++){
				// 指派按钮创建的时候 就创建menubutton按钮，防止后期改造造成抖动	
				if(buttons[i] && buttons[i].text ===  Edoc2FormSR["Flow_Assign"] ){
					buttons[i].menu = true;
					buttons[i].iconCls = "icon-assign";
					buttons[i].menuStr = "#assignMenu";
				}
			}
		});
	}
	...
	if(eform.pageType == "pc") {
		...
		if(zp) {
			...
			var param2 = {
				...
				target: eform.virtualPath + "/Default/Default?skin="+skin+"&formId=150909140122", // %u5730%u5740
			...
			$("#assignMenu").menu('appendItem', {
				...
				onclick: function() {
					...
					param3.target = eform.virtualPath + "/Default/Default?skin="+skin+"&formId=150909171524";
			...
			$("#assignMenu").menu('appendItem', {
				...
				onclick: function() {
					var param3 = $.extend(true, {}, param2);
					var config1 = $.extend(true, {}, config);
					param3.target = eform.virtualPath + "/Default/Default?skin="+skin+"&formId=150910104413";
					config1.width = dialogWidth;
					config1.height = dialogHeight;
		...
		if(claimantBtn) {
			$(claimantBtn).unbind().bind('click', function() {
				var param3 = $.extend(true, {}, param2);
				var config1 = $.extend(true, {}, config);
				param3.target = eform.virtualPath + "/Default/Default?skin="+skin+"&formId=161205111902";
				config1.width = dialogWidth;
				config1.height = dialogHeight;
		...
		if(batchApprovalBtn){
			...
			$(batchApprovalBtn).unbind().bind('click', function() {
				
				var param3 = $.extend(true, {}, param2);
				var config1 = $.extend(true, {}, config);
				param3.target = eform.virtualPath + "/Default/Default?skin="+skin+"&formId=181116135227";
				
				
				param3.onBeforOpen = function(){  // 窗口打开前事件,return false不打开窗体
					
					var easyGrid = eform("edoc2ListGrid_myInbox").method("getEasyControl");
					var selectedRows = easyGrid.datagrid("getSelections");
				...
				config1.title = Edoc2FormSR["BatchApproval"];
				var batchApprovalDialog = new eform.Dialog(config1, param3);
				window.batchApprovalDialog = batchApprovalDialog;
