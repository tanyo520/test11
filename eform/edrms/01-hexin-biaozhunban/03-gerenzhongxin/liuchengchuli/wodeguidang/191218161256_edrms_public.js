...
window.myExtCode.jumpUrl = function(type, proccessId, taskId, incidentId, businessKey,state) {
	...
	$.ajax({
		type: "get",
		url:"/edoc2Flow-web/edoc2-form/jumpIndex?orgToken="+$.cookie("token")+"&isArchiveTask=true&t=" + new Date().getTime(),
		async: true,
		data: data,
...
window.myExtCode.drawBefore = function(){
	var efListGrid = eform("edoc2ListGrid_myArchive");
	...
	efListGrid.method("customColumnsFormatter", function (value, row, index, field, fieldName) {
		if(eform.pageType == "pc"){
			if(field == "Process_Name"){
				return "<a class='blackA' href=\"#\" onclick=\"javascript:window.myExtCode.jumpUrl('ArchiveTask','" + row.Process_ID + "','" + row.Task_ID + "','" + row.Incident_ID + "','" + row.Process_FormKey + "')\">"+value+"</a>";
			}
			else if(field == "Num"){
			...
			if(field == "Process_Name"){
				return "<a href=\"#\" onclick=\"javascript:window.myExtCode.jumpUrl('ArchiveTask','" + row.Process_ID + "','" + row.Task_ID + "','" + row.Incident_ID + "','" + row.Process_FormKey + "')\" style='color:#009cd0'>"+value+"</a>";
			}
			else if(field == "Task_State"){
				return window.myExtCode.setStateImage(row);
			}
			else if(field == "Incident_CreateTime"){
				return eform.getDateInterval(value);
			}
			else if(field == "Incident_CompleteTime"){
				return eform.getDateInterval(value);
			}
		}
		return value;
...
window.myExtCode.parserBefore = function(){
	//grid draw before
	eform("edoc2ListGrid_myArchive").method("drawBefore",function () {	
...
window.myExtCode.parserAfter = function(){
	if(eform.pageType == "pc"){
// 		var easyCtrl = eform("edoc2ListGrid_myArchive").method("getEasyControl");
// 		var options = easyCtrl.datagrid("options");
// 		options.type="small";
// 		easyCtrl.datagrid(options);
// 		eform("edoc2ListGrid_myArchive").method("hideSearchBlock");
