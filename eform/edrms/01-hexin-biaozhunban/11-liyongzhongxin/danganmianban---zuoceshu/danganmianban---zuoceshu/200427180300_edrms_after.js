...
$(document).on("click", ".linkArch", function (value) {
    ...
    pathName = pathName.replace(pathArray[3], "danganxiangqingye");
    window.parent.window.open(window.parent.location.origin + pathName + "?archtypeid=" + localStorage.getItem("archTypeId") + "&id=" + value.target.id);
...
function loadData(archList, curPage) {
    ...
    for (var i = startIndex; i < whileLength; i++) {
        ...
        var divShoucang = '<div class="rowButton shoucang" id="' + rowData.Id + '"><img id="' + rowData.favoId + '" src="' + shoucangSrc + '"></div>';
        ...
        var divJieyue = '<div class="rowButton jieyueche "  id="' + rowData.Id + '"><img id="'+rowData.borrowcarId+'" src="'+jieyuecheSrc+'"></div>';
        $('#row' + i + ' .rowButtonParent').append(divJieyue);

        var divJieyueche = '<div class="rowButton jieyue "  id="' + rowData.Id + '"><img src="/resourcefiles/31ec7ad5-f053-453b-8258-91c3f7151afa.png?system_var_rootId=$system_var_rootId$"></div>';
