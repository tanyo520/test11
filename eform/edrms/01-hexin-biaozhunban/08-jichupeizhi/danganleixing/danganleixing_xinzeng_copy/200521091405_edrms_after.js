...
* oldValue 旧值
*/
eform("arch_strategy_id").method("onChange", function(newValue, oldValue) {
    eform.dataset("selectById", {Id:newValue,tableName:'strategy'}, function(result) {
        var details = result.Data[0][0].details;
        eform("arch_number_strategy").method("setValue",details);
    }, false);
});
eform("dossier_strategy_id").method("onChange", function(newValue, oldValue) {
    eform.dataset("selectById", {Id:newValue,tableName:'strategy'}, function(result) {
        var details = result.Data[0][0].details;
...
eform("eformButton1").method("onClick", function (buttonName, buttonId) {
	...
	//档案固定字段
	var staticFields="entrystate,ifDossiered,dossierId,deadTime,secertexp,reorganizer,reorganizedate,sectid,archtypeid,archiver,archivername,archivedate,";
