...
$('#eformHidden1').parent().append('<div style="margin:0px  20px;height:'+(height-150)+'px;overflow:auto;"><ul id="dictTree"></ul></div>');
...
eform("eformButton1").method("onClick", function (buttonName, buttonId) {
	...
	if(buttonName == "新增字典"){//新增字典
		...
		var src = window.location.protocol+"//"+window.location.host+"/eform/index?formid="+formId+"&skin=techblue&pId="+nodeid+"&level="+level;
		console.log(src)
		$(top.document).find("iframe").eq(1).attr("src",src);
