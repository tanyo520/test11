...
eform("eformDataTable1").method("onUrlFormatter",function (url,operateBtn) {
    ...
    if (operateBtn.formName == "新增"){
        if(!eform("data_id").method('getValue')){
            ...
            var newFormUrl = eform.virtualPath + '/index?formid=200220100923_edrms&skin=techblue&viewtype=0';
            return newFormUrl;
        }
    }
...
eform("eformDataTable1").method("customButtonEvent", function (formId, ids, name, callback) {
    ...
    if (name == "删除") {
        ...
        for(var i=0;i<rows.length;i++){
            ...
            var MemberType=1;
            if(rows[i].perm_type==5){
                MemberType = 2
            }
            else if(rows[i].perm_type==4){
                MemberType = 4
            }
            else if(rows[i].perm_type==3){
                MemberType = 8
            }
