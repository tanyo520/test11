...
//自定义一个全宗档案类型树
$('#eformHidden1').parent().append('<div style="margin:0px;height:'+(height-5)+'px;overflow:auto;"><ul id="sectArchTypeTree"></ul></div>');
$('.formBlock ').css('margin-top','0px');//去除全宗档案类型树上方空白
...
$('#sectArchTypeTree').tree({
	...
	,onClick: function(node){
		...
		var src = "/eform/index?formid="+formId+"&skin=techblue&Id="+node.id+"&pId="+node.pId
		+"&rootId="+root.id+"&isSect="+node.isSect+"&parentIds="+parentIds+"&name="+encodeURI(node.text)+"&folderId="+folderId
